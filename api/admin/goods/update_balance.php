...
$good = mysqli_query($connect, "SELECT `balance`, `id_warehouse` FROM `goods` WHERE `id` = $id");
$good = mysqli_fetch_assoc($good);
$current_balance = $good['balance'];
$warehouse = $good['id_warehouse'];

mysqli_query($connect, "UPDATE `goods` SET `balance` = $balance WHERE `id` = $id");
$req = [
    "messages" => ["Остаток фасованного товара успешно изменен"],
    "balance" => $balance
];
