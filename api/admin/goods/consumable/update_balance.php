...
$good = mysqli_query($connect, "SELECT `balance`, `id_warehouse` FROM `goods_consumable` WHERE `id` = $id");
$good = mysqli_fetch_assoc($good);
$current_balance = $good['balance'];
$warehouse = $good['id_warehouse'];

mysqli_query($connect, "UPDATE `goods_consumable` SET `balance` = $balance WHERE `id` = $id");
$req = [
    "messages" => ["Остаток расходника успешно изменен"],
    "balance" => $balance
];
